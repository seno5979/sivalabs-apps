...
public class TxnRepeatableReadIsolationLevelsTest
{
		...
		Thread t1 = new Thread(new Runnable() {
			...
			public void run()
			{
				try 
				{
					...
					PreparedStatement pstmt1 = conn1.prepareStatement("select salary from employee where emp_id=1");
					ResultSet rs = pstmt1.executeQuery();
					if(rs.next()){
						System.out.println("Salary for emp_id=1 for first time is :"+rs.getDouble(1));
					}
					Thread.sleep(1000);
					pstmt1 = conn1.prepareStatement("select salary from employee where emp_id=1");
					rs = pstmt1.executeQuery();
					if(rs.next()){
						System.out.println("Salary for emp_id=1 for second time is :"+rs.getDouble(1));
					}
					conn1.commit();
		...
		Thread t2 = new Thread(new Runnable() {
			...
			{				
				try 
				{
					...
					Thread.sleep(100);
					PreparedStatement pstmt2 = conn2.prepareStatement("update employee set salary = salary + 500 where emp_id=1");
